...
*Happy-path trace (adding a book to a reading list):* A trace for `POST /api/reading-lists/1/books` shows multiple spans across three services. The API Gateway spans cover JWT validation and routing. The Catalog Service spans cover business logic and the outbound Feign call. The Library API spans show the book verification. Each service's contribution to the total request duration is visible, confirming that inter-service latency is minimal and the Feign call to Service B is the largest contributor to response time — important context for capacity planning.
